(() => {
  ...
  function buildDefenderStartupPreset() {
    ...
    const p0 = mk();
    p0.mode = "texture";
    p0.textureBehavior = "sustain";
    p0.engineEnabled = true;
    p0.clockRate = +RNG.range(4.2, 8.4).toFixed(2);
    p0.gateDepth = +RNG.range(0.34, 0.58).toFixed(2);
    p0.stepAmount = +RNG.range(0.28, 0.56).toFixed(2);
    p0.delayMix = +RNG.range(0.12, 0.28).toFixed(2);
    p0.delayTime = +RNG.range(0.08, 0.16).toFixed(3);
    p0.feedback = +RNG.range(0.22, 0.5).toFixed(2);
    p0.oscType = RNG.pick(["square", "sawtooth"]);
    p0.singleOsc = false;
    p0.pwmOn = p0.oscType === "square";
    p0.pwm = +RNG.range(0.24, 0.68).toFixed(2);
    p0.oscRate = +RNG.range(0.9, 1.18).toFixed(2);
    p0.baseHz = Math.round(RNG.range(220, 320));
    p0.unisonSpread = +RNG.range(0.0012, 0.0065).toFixed(4);
    p0.detune = Math.round(RNG.range(2, 14));
    p0.subMix = +RNG.range(0.08, 0.22).toFixed(2);
    p0.drive = +RNG.range(0.28, 0.56).toFixed(2);
    p0.filterCutoff = Math.round(RNG.range(1400, 2600));
    p0.filterQ = +RNG.range(0.9, 2.0).toFixed(1);
    p0.edge = +RNG.range(0.18, 0.42).toFixed(2);
    p0.noiseMix = +RNG.range(0.0, 0.09).toFixed(2);

    const p1 = mk();
    p1.mode = "texture";
    p1.textureBehavior = "sustain";
    p1.engineEnabled = true;
    p1.clockRate = +RNG.range(3.0, 6.8).toFixed(2);
    p1.gateDepth = +RNG.range(0.2, 0.45).toFixed(2);
    p1.stepAmount = +RNG.range(0.14, 0.42).toFixed(2);
    p1.delayMix = +RNG.range(0.08, 0.22).toFixed(2);
    p1.delayTime = +RNG.range(0.11, 0.24).toFixed(3);
    p1.feedback = +RNG.range(0.12, 0.38).toFixed(2);
    p1.oscType = RNG.pick(["sawtooth", "triangle", "square"]);
    p1.singleOsc = RNG.next() < 0.25;
    p1.pwmOn = p1.oscType === "square";
    p1.pwm = +RNG.range(0.22, 0.72).toFixed(2);
    p1.oscRate = +RNG.range(1.0, 1.34).toFixed(2);
    p1.baseHz = Math.round(RNG.range(280, 460));
    p1.unisonSpread = +RNG.range(0.0008, 0.0048).toFixed(4);
    p1.detune = Math.round(RNG.range(0, 10));
    p1.subMix = +RNG.range(0.0, 0.15).toFixed(2);
    p1.drive = +RNG.range(0.16, 0.44).toFixed(2);
    p1.filterCutoff = Math.round(RNG.range(1800, 3600));
    p1.filterQ = +RNG.range(0.7, 1.8).toFixed(1);
    p1.edge = +RNG.range(0.1, 0.34).toFixed(2);
    p1.noiseMix = +RNG.range(0.0, 0.07).toFixed(2);

    const p2 = mk();
    p2.mode = "noise";
    p2.noiseBehavior = "sustain";
    p2.engineEnabled = true;
    p2.clockRate = +RNG.range(5.4, 11.2).toFixed(2);
    p2.gateDepth = +RNG.range(0.36, 0.7).toFixed(2);
    p2.stepAmount = +RNG.range(0.28, 0.62).toFixed(2);
    p2.delayMix = +RNG.range(0.06, 0.22).toFixed(2);
    p2.delayTime = +RNG.range(0.07, 0.18).toFixed(3);
    p2.feedback = +RNG.range(0.16, 0.44).toFixed(2);
    p2.noiseType = RNG.pick(["bit", "metallic", "white"]);
    p2.noisePeak = +RNG.range(0.68, 0.96).toFixed(2);
    p2.noiseLPStart = Math.round(RNG.range(3200, 9000));
    p2.noiseLPEnd = Math.round(RNG.range(300, 1300));
    p2.noiseSweepTime = +RNG.range(0.55, 1.8).toFixed(2);
    p2.noiseHPF = Math.round(RNG.range(120, 760));
    p2.noiseRes = +RNG.range(0.8, 4.8).toFixed(1);
    p2.noiseBoomHz = Math.round(RNG.range(26, 80));
    p2.noiseBoomAmt = +RNG.range(0.12, 0.62).toFixed(2);
    p2.noiseBoomDrop = +RNG.range(0.28, 0.82).toFixed(2);
    p2.noiseRingHz = Math.round(RNG.range(180, 980));
    p2.noiseRingAmt = +RNG.range(0.16, 0.52).toFixed(2);
    p2.noiseRingDrop = +RNG.range(0.2, 0.75).toFixed(2);
    p2.noiseCrackleAmt = +RNG.range(0.16, 0.54).toFixed(2);
    p2.noiseCrackleHPF = Math.round(RNG.range(1200, 4200));
    p2.noiseEdgeDrive = +RNG.range(0.32, 0.86).toFixed(2);
    p2.noiseVolume = +RNG.range(0.92, 1.12).toFixed(2);

    const p3 = mk();
    p3.mode = "bass";
    p3.bassBehavior = "sustain";
    p3.engineEnabled = true;
    p3.clockRate = +RNG.range(1.8, 4.8).toFixed(2);
    p3.gateDepth = +RNG.range(0.1, 0.32).toFixed(2);
    p3.stepAmount = +RNG.range(0.08, 0.32).toFixed(2);
    p3.delayMix = +RNG.range(0.0, 0.18).toFixed(2);
    p3.delayTime = +RNG.range(0.08, 0.2).toFixed(3);
    p3.feedback = +RNG.range(0.0, 0.32).toFixed(2);
    p3.bassHz = Math.round(RNG.range(44, 72));
    p3.bassLP = Math.round(RNG.range(110, 240));
    p3.bassRes = +RNG.range(0.6, 2.4).toFixed(1);
    p3.bassDrive = +RNG.range(0.08, 0.42).toFixed(2);
    p3.bassLfoPitch = +RNG.range(0.05, 0.22).toFixed(2);
    p3.bassLfoPitchRate = +RNG.range(0.06, 0.26).toFixed(2);
    p3.bassLfoLPF = +RNG.range(0.06, 0.26).toFixed(2);
    p3.bassLfoLPFRate = +RNG.range(0.05, 0.24).toFixed(2);
    p3.bassLfoRes = +RNG.range(0.02, 0.18).toFixed(2);
    p3.bassLfoResRate = +RNG.range(0.03, 0.22).toFixed(2);
    p3.bassLfoDrive = +RNG.range(0.03, 0.18).toFixed(2);
    p3.bassLfoDriveRate = +RNG.range(0.03, 0.22).toFixed(2);
    p3.bassDrift = +RNG.range(0.04, 0.2).toFixed(2);
    p3.bassVolume = +RNG.range(0.96, 1.15).toFixed(2);
