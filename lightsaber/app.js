(() => {
  ...
  const defaults = () => {
    ...
    return {
      ...
      bassBoost: +RNG.range(3.0, 8.0).toFixed(1),
  ...
  function randomizeVoice(p, randomizeConfig) {
    ...
    p.bassBoost = +RNG.range(2.0, 12.0).toFixed(1);
  ...
  function createVoice(ctx, master, noiseBuf) {
    ...
    function apply(p, muted, width, basePan, opts = {}) {
      ...
      const bassBoost = clamp(Number(p.bassBoost) || 4.0, 1.0, 12.0);
      const bassGainValue = Math.pow(bassGainNorm, 1.6) * 1.2;
      ...
      if (isBass) {
        ...
        bassPostGain.gain.setTargetAtTime(bassBoost, t, 0.08);
  ...
  function initVoices() {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (p.bassBoost == null) p.bassBoost = 4.0;
  ...
  function formatValue(key, value) {
    ...
    if (key === "bassBoost") return `${value.toFixed(1)}x`;
